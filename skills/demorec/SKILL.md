...
#### Sharing Demos in Pull Requests

The recommended approach for PR demos uses a `.pr/` directory with an HTML viewer page. This provides:
- Direct video playback without downloading
- Works via GitHub's htmlpreview service
...
└── frames/                 # Optional: frame captures for debugging
    ├── frame_0001_000.00.txt
    └── ...
```
...
**[▶️ View Demo Page](https://htmlpreview.github.io/?https://github.com/USER/REPO/blob/BRANCH/.pr/index.html)** | **[📥 Download Video](https://github.com/USER/REPO/raw/BRANCH/.pr/demo.mp4)**

XX-second video demonstrating:
- Feature 1
- Feature 2
- N mode switches / segments
...
Create `.pr/index.html` with video embed and feature highlights:
...
For simpler demos, you can drag the video directly into a GitHub comment. GitHub will upload and embed it automatically. However, this has size limits and doesn't provide the rich presentation context.

#### Other Sharing Options

- **Release notes**: Embed video or link to demo page
- **Documentation**: Include demo links with context
- **Slack/Teams**: Share the htmlpreview link for easy viewing
- **Accessibility**: The `.srt` subtitle file generated by demorec can be embedded or provided alongside
...
# Install Marp CLI for presentation mode (optional, only if using slides)
...
|------------|---------|----------|
| Playwright + Chromium | Browser automation and screen capture (terminal + browser modes) | Yes |
| ttyd | Real PTY support for terminal recording (actual shell execution) | Yes |
| ffmpeg | Video segment concatenation, audio mixing, subtitle embedding | Yes |
| vim | Code review demos with syntax highlighting | For code demos |
| Marp CLI | Presentation mode with slides | For `@mode presentation` |
...
demorec uses [Marp](https://marp.app/) for presentation mode. You can use local files, URLs, or the built-in OpenHands theme.

#### Installation

```bash
# Marp CLI is required for presentation mode
npm install -g @marp-team/marp-cli
```

#### Presentation Mode Syntax

```tape
# Local Marp file
@mode presentation "slides.md"

# Remote Marp file (HTTP URL)
@mode presentation "https://raw.githubusercontent.com/user/repo/main/slides.md"

# Set theme (local file, URL, or alias)
Set Theme "openhands"      # Built-in OpenHands theme
Set Theme "default"        # Marp default theme
Set Theme "./custom.css"   # Local theme file
Set Theme "https://..."    # Remote theme URL
```

#### Using the OpenHands Theme

The `openhands` theme alias points to the official OpenHands Marp theme with branded styling:

```tape
@mode presentation "https://raw.githubusercontent.com/jpshackelford/marp-intro/main/examples/openhands-intro/openhands-intro.md"
Set Theme "openhands"

# @narrate:during "Welcome to OpenHands."
Slide 1 4s

# @narrate:during "Let me show you how to get started."
Slide 2 3s
```

#### Writing Marp Slides

Create a standard Marp markdown file:
...
Presentation mode uses smart timing: `max(min_duration, narration_duration + padding)`. If your narration is 5 seconds but you set `Slide 1 3s`, it will wait for the full narration plus a small buffer.
