...
gh issue list --repo {REPOSITORY} --label "ready" --json number,title,labels,body --jq '.[] | {number, title, labels: [.labels[].name]}'
```
...
```bash
gh issue view {number} --repo {REPOSITORY} --comments
```
...
gh issue edit {number} --repo {REPOSITORY} --add-label "priority:high"

# Can also remove old priority if reassessing
gh issue edit {number} --repo {REPOSITORY} --remove-label "priority:medium" --add-label "priority:high"
```
