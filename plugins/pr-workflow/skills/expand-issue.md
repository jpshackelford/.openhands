...
# Update issue body
gh issue edit {number} --repo {REPOSITORY} --body "new body here"

# Or use body from file
gh issue edit {number} --repo {REPOSITORY} --body-file /tmp/issue-body.md

# Add a comment
gh issue comment {number} --repo {REPOSITORY} --body "comment here"

# Add ready label
gh issue edit {number} --repo {REPOSITORY} --add-label "ready"

# Remove needs-triage label if present
gh issue edit {number} --repo {REPOSITORY} --remove-label "needs-triage"
```
...
- Issue: [{title}](https://github.com/{REPOSITORY}/issues/{number})
