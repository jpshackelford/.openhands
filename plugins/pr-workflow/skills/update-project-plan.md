...
gh issue list --repo {REPOSITORY} --state open --json number,title
```
...
```bash
gh issue comment {number} --repo {REPOSITORY} --body "**Note from Issue #X implementation:**
...
```bash
gh issue comment 18 --repo {REPOSITORY} --body "**Note from Issue #15 (Add --json output):**
- The output formatting is handled in \`src/formatters.py\`
...
For CLI tools, common learnings to capture include:
