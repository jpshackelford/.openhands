...
### Step 1: Read the issue and identify the failure context
...
### Step 3: Triage the failure mode

Classify the failure into one of:

| Failure mode | Definition | Action |
|---|---|---|
| **Real regression** | The failing commit's code change is the responsible party. A revert or forward fix changes behavior. | Step 4 (forward fix) or Step 5 (revert). |
| **Flaky test** | The test is non-deterministic; the failing commit is unrelated. Evidence: re-running the same workflow on the same commit produces a different result. | Step 6 (flaky verdict). |
| **Test infra issue** | The test or deploy infrastructure broke independently of code. Evidence: similar smoke tests on unrelated PRs around the same time also failed, or the failure is in setup/teardown not in app code. | Step 7 (infra verdict). |
| **Deferred work blocking smoke** | The failing commit is correct but the smoke test exercises a code path that depends on a deferred follow-up. Evidence: the failing test exercises behavior listed in a `## Deferred to follow-ups` item. | Step 8 (deferral verdict). |

Don't agonize over the classification — pick the most likely and proceed. If the next step's evidence contradicts the classification, come back here and re-classify.
...
### Step 7: Test infra issue verdict
...
2. If the fix is within scope of this repo (e.g. a workflow YAML change), proceed as in Step 4 with a `chore(ci):` PR.
...
2. Two options:
   ...
   - **Mark the original PR's responsible commit for revert** (Step 5).
...
- Failure mode classification (one of the four).
