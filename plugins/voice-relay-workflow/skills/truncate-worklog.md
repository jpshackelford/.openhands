...
```bash
# Read the file
WORKLOG_CONTENT=$(cat WORKLOG.md)

# Extract header (everything before "## Log" line, inclusive)
HEADER=$(echo "$WORKLOG_CONTENT" | sed -n '1,/^## Log$/p')

# Extract entries section (everything after "## Log")
...
def parse_entries(entries_section: str) -> list[dict]:
    """Parse worklog entries into structured data."""
    entries = []
    ...
    for raw in raw_entries:
        ...
        # Extract timestamp from header: ### 2026-05-06 19:15 UTC - ...
...
def is_productive(content: str) -> bool:
    ...
    # Check for status indicators first (they override)
    ...
    # Default: treat as status (conservative)
    return False
```
...
def calculate_retention(entries: list[dict], min_productive_hours: int = 6) -> int:
    """
    Calculate the index of the oldest entry to keep.
    
    Returns the index in the entries list (sorted oldest→newest) 
    where we should start keeping entries.
    """
    if not entries:
        return 0
    ...
    # Find productive entries
    productive_entries = [e for e in sorted_entries if e['productive']]
    
    if not productive_entries:
        # No productive work at all - keep everything
        return 0
    
    newest_productive = productive_entries[0]['timestamp']
    oldest_in_window = newest_productive
    
    # Expand window until we have 6 hours of productive span
    for entry in productive_entries:
        oldest_in_window = entry['timestamp']
        span = newest_productive - oldest_in_window
        if span >= timedelta(hours=min_productive_hours):
            break
    
    # The cutoff is the oldest productive entry in our window
    # But we also keep all entries (including status) from that point forward
    cutoff_ts = oldest_in_window
    
    # Find the index in the original (oldest→newest) sorted list
    entries_oldest_first = sorted(entries, key=lambda e: e['timestamp'])
...
def archive_entries(entries_to_archive: list[dict], repo_path: str):
    ...
    for date, date_entries in by_date.items():
        ...
        # Sort entries oldest first within each day
        ...
        if os.path.exists(archive_file):
            ...
            header = f"""# Voice Relay Worklog Archive - {date}
...
def rewrite_worklog(header: str, kept_entries: list[dict], worklog_path: str):
    """Rewrite WORKLOG.md with header and kept entries."""
    # Sort entries oldest first (chronological order in file)
    kept_entries.sort(key=lambda e: e['timestamp'])
    
    entries_content = "\n\n---\n".join(e['content'] for e in kept_entries)
    ...
    print(f"Kept {len(kept_entries)} entries in WORKLOG.md")
```
...
def truncate_worklog(repo_path: str = ".", min_hours: int = 6, dry_run: bool = False):
    ...
    if kept_productive:
        span = kept_productive[0]['timestamp'] - kept_productive[-1]['timestamp']
        # Sort is newest first after cutoff filtering
...
import sys
sys.path.insert(0, '.')
exec(open('.agents/scripts/truncate_worklog.py').read())
truncate_worklog('.', min_hours=6)
"
fi
```
