...
### Step 1: Clone the repo, checkout the PR branch
...
### Step 2: IMMEDIATELY drop the PR back to draft
...
### Step 3: Read ALL review threads carefully
...
### Step 4: Plan the response per thread

For each thread, decide:

- **Accept and implement** — most suggestions improve quality; the default.
- **Reject with explanation** — only when the suggestion significantly increases scope/complexity without clear benefit. Write the explanation now; you'll post it in Step 7.

### Step 5: Group related changes into logical commits

Don't bundle everything into one commit. The reviewer should be able to read your response commit-by-commit.

### Step 6: For each commit
...
### Step 8.5: Re-run the Closing-Trailer AC Gate (REQUIRED)
...
3. **Compare to the previous verdict** recorded in the PR body's `## Deferred to follow-ups` section (or absence thereof):
   ...
   | `Fixes #N` (all satisfied) | Now some unsatisfied | **Downgrade trailer to `Refs #N`**, file follow-up issues for the new gaps, add a `## Deferred to follow-ups` section. Same procedure as `/implement-issue` Step 9 fail path. |
...
4. **Note the re-verdict** in the comment that closes out this review round (Step 9). Be explicit: "AC gate re-run: verdict unchanged" or "AC gate re-run: now `Refs #386 + 2 follow-ups #408, #409` (was `Fixes #386`); reviewer asked us to drop the workspace-settings UI from this PR."

### Step 9: Move PR back to ready
...
### Step 10: WORKLOG entry on main

Update `WORKLOG.md` on `main` with:

- PR link + review round number
- Summary of threads addressed
- Gate re-verdict from Step 8.5 (one of: `unchanged`, `now refs + N follow-ups`, `now closes`)
