...
Then provide:
- **repository**: GitHub repo (e.g., `jpshackelford/voice-relay`)
...
gh pr list --repo jpshackelford/voice-relay --state open --json number
...
```
 ...
 ───────────────────────────────────────────────────────────────────────────────────
  jpshackelford/voice-relay   #1   oCR       green   ready    2   18h   15h ago
```
...
```bash
lxa review --repo jpshackelford/voice-relay
```
...
PR_NUM=$(gh pr list --repo jpshackelford/voice-relay --state open --json number -q '.[0].number')

# 2. Quick status check
lxa pr list "jpshackelford/voice-relay#${PR_NUM}"

# Output: oCR green ready 2 - needs attention (2 unresolved threads)

# 3. Get details on what needs addressing
gh pr view ${PR_NUM} --repo jpshackelford/voice-relay --comments
