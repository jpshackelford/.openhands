...
gh pr diff PR_NUMBER --repo jpshackelford/lxa

# Read the PR description and all comments
gh pr view PR_NUMBER --repo jpshackelford/lxa --comments
```
...
```bash
gh pr view PR_NUMBER --repo jpshackelford/lxa --comments | grep -A100 "Manual Test Results"
```
...
```bash
gh pr merge PR_NUMBER --repo jpshackelford/lxa --squash \
...
gh pr view PR_NUMBER --repo jpshackelford/lxa --json state,mergedAt
