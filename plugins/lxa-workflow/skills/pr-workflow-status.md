...
Then provide:
- **repository**: GitHub repo (e.g., `jpshackelford/lxa`)
...
gh pr list --repo jpshackelford/lxa --state open --json number

# List specific PR (use the discovered number)
lxa pr list "jpshackelford/lxa#<PR_NUMBER>"

# List all open PRs for a repo (need to add to board first)
lxa repo add jpshackelford/lxa
lxa pr list --all
```
...
For lxa, manual testing is **required** before code review. Check if test results have been posted:

```bash
# Simple check via grep
gh pr view PR_NUMBER --repo jpshackelford/lxa --comments | grep -i "Manual Test Results"
...
' -f owner=jpshackelford -f repo=lxa -f number=42 | \
  jq '.data.repository.pullRequest.comments.nodes[] | select(.body | test("Manual Test Results"; "i"))'
```

## Determine PR Phase

Based on gathered information, determine where the PR is in the lxa workflow:
...
gh pr list --repo jpshackelford/lxa --state open --json number,title,isDraft

# 2. Quick status check for each
lxa pr list "jpshackelford/lxa#42"
# Output: oC green ready 2 - needs attention

# 3. Check for manual test results
gh pr view 42 --repo jpshackelford/lxa --comments | grep -c "Manual Test Results"
# Output: 1 (test results present) or 0 (not tested yet)

# 4. If tested, check review status
gh pr view 42 --repo jpshackelford/lxa --comments | tail -50
```
