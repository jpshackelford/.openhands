...
gh issue list --repo jpshackelford/lxa --label "ready" --json number,title,labels,body --jq '.[] | {number, title, labels: [.labels[].name]}'
```
...
```bash
gh issue view {number} --repo jpshackelford/lxa --comments
```
...
gh issue edit {number} --repo jpshackelford/lxa --add-label "priority:high"

# Can also remove old priority if reassessing
gh issue edit {number} --repo jpshackelford/lxa --remove-label "priority:medium" --add-label "priority:high"
```
