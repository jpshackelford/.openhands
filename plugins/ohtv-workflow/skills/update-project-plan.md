...
gh issue list --repo jpshackelford/ohtv --state open --json number,title
```
...
```bash
gh issue comment {number} --repo jpshackelford/ohtv --body "**Note from Issue #X implementation:**
...
```bash
gh issue create --repo jpshackelford/ohtv \
...
```bash
gh issue comment 18 --repo jpshackelford/ohtv --body "**Note from Issue #15 (Add --json output):**
- The output formatting is handled in \`src/ohtv/formatters.py\`
...
For ohtv as a CLI tool, common learnings to capture include:
