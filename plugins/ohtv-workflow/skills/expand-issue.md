...
**Location:** `src/ohtv/path/to/file.py` lines 42-58
...
**Files to modify:**
- `src/ohtv/path/to/file.py` - [what changes]
...
- `src/ohtv/path/to/file.py` - [what changes]
- `src/ohtv/new_file.py` - [new file if needed]
- `tests/...` - [test files]
...
```bash
# Read the issue
gh issue view {number} --repo jpshackelford/ohtv

# Update issue body
gh issue edit {number} --repo jpshackelford/ohtv --body "new body here"

# Or use body from file
gh issue edit {number} --repo jpshackelford/ohtv --body-file /tmp/issue-body.md

# Add a comment
gh issue comment {number} --repo jpshackelford/ohtv --body "comment here"

# Add ready label
gh issue edit {number} --repo jpshackelford/ohtv --add-label "ready"

# Remove needs-triage label if present
gh issue edit {number} --repo jpshackelford/ohtv --remove-label "needs-triage"
```
...
- Issue: [{title}](https://github.com/jpshackelford/ohtv/issues/{number})
