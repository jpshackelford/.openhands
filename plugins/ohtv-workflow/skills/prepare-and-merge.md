...
gh pr diff PR_NUMBER --repo jpshackelford/ohtv

# Read the PR description and all comments
gh pr view PR_NUMBER --repo jpshackelford/ohtv --comments
```
...
```bash
gh pr view PR_NUMBER --repo jpshackelford/ohtv --comments | grep -A100 "Manual Test Results"
```
...
```bash
gh pr merge PR_NUMBER --repo jpshackelford/ohtv --squash \
...
gh pr view PR_NUMBER --repo jpshackelford/ohtv --json state,mergedAt
